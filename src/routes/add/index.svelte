...
$: quoteBody, authorName, authorTitle, context, tags, source
	...
	function checkIfCurrentQuoteExistsAndPopulateForm() {
		...
		// if(q){
		// 	console.log(`🚀 ~ file: AddQuote.svelte ~ line 60 ~ checkIfCurrentQuoteExistsAndPopulateForm ~ q `, q )
		// 	fieldQuoteBody.value = q.quoteBody;
		// 	fieldAuthorName.value = q.authorName;
		// 	fieldAuthorTitle.value = q.authorTitle;
		// 	fieldContext.value = q.context;
		// 	fieldTags.value = q.tags;
		// 	fieldSource.value = q.source;
		// }
		for (const textareaElement of textareaElements) {
				new TextareaAutoResize(textareaElement);
			}
...
<div class="p-10 card bg-slate-800">
	<div class="form-control flex  max-w-[600px]">
